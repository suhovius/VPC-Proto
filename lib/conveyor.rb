  ...
  class << self
    def perform(encoding_id)
      ...
      encoding.profile.commands.asc(:ordering_number).each do |command|
        processing_job = initiate_job_by command, params
    ...
    def prepare_options_for command, params
      ...
      command.options.each do |option|
        value = option.params_key_name ? params[option.params_key_name] : option.value
        prepared_options.[]= option.key.to_sym, value
      end
      prepared_options
    end

    def initiate_job_by command, params
      job_klass = "Job::#{command.job_name.camelize}".constantize
      storage = TempStorage.new encoding_id, command.ordering_number
      job_klass.new set_input_data_for(command), storage.output_dir_fullpath, prepare_options_for(command, params)
    end
  end
